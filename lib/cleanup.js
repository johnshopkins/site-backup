...
cleanup.prototype.connectS3 = function (config) {
  ...
  this.sdk = aws.connect(config);
  this.s3 = new this.sdk.S3();
  return this;

};

cleanup.prototype.cleanup = function (bucket, dbprefix) {

  var cleaner = this;

  cleaner.dbprefix = dbprefix || "";
  ...
  cleaner.getObjects(bucket, function (error, objects) {
    ...
    if (toDelete.length === 0) {
      console.log("No database backups to cleanup. Exiting");
      return;
    }
    ...
    cleaner.deleteObjects(bucket, toDelete, function (error, data) {
      ...
      console.log("Old database backups removed from s3.");
...
cleanup.prototype.analyzeObject = function (filename) {
  ...
  var prefix = this.dbprefix ? this.dbprefix + "_" : "";
